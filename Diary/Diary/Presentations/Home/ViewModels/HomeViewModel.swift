...
final class DiaryStore: ObservableObject {
    ...
    // SwiftData에서 fetch된 DiaryModelData로부터 업데이트
    func update(with models: [DiaryModelData]) {
        self.entries = models.map { model in
            DiaryEntry(
                date: model.createDate,
                content: model.diaryContentSummary,
                quote: model.wiseSayingSummary,
                vow: model.resolutionSummary,
                reflection: model.retrospectiveSummary
            )
        }
    }
}
